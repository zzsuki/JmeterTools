...
import com.meterspheretools.utils.ListUtil;
import com.meterspheretools.utils.ByteUtil;
...
public abstract class AbstractCipher {
     ...
     */
    public AbstractCipher(String key, int contentSize){
        this.key = key;
     ...
     */
    protected String doFinalEncrypt(String word, String transformation, String key){
        try {
            // 获取并初始化cipher
            Cipher cipher = this.getCipher(transformation);
            cipher.init(Cipher.ENCRYPT_MODE, this.getKeySpec(key));
            // 获取加密结果（byte 数组）
     ...
     * @return 解密后的字符串
     */
    protected String doFinalDecrypt(String word, String transformation, String key){
        try {
            // 获取并初始化cipher
            Cipher cipher = this.getCipher(transformation);
            cipher.init(Cipher.DECRYPT_MODE, this.getKeySpec(key));
            // 将16进制字符串转为byte数组
